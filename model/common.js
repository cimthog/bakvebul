...
module.exports.getallcat=function(language,cb)
{
...
module.exports.getAllArtist=function(user_pub_id,cat_id,cb)
{
    ...
    var que="SELECT cs.currency_symbol,multcat.user_pub_id,multcat.cat_id,cat.cat_name,us.*,concat('"+base_url+"',us.image) as image, concat('',cat.id) AS cat_id from user us JOIN `currency_setting` AS cs ON cs.id = us.currency_id JOIN `multiple_category` AS multcat ON multcat.user_pub_id = us.user_pub_id JOIN `categories` AS cat ON cat.id = multcat.cat_id WHERE us.status = '1' AND us.is_artist = '1' AND us.user_pub_id !='"+user_pub_id+"' AND multcat.cat_id='"+cat_id+"' AND (us.sub_end_date != '' AND us.sub_end_date > ROUND(UNIX_TIMESTAMP()*1000))"; 
    con.query(que,cb);
}
module.exports.getAllArtist1=function(user_pub_id,cb)
{
    // var que = 'select u.*,c.cat_name from multiple_category u join categories c on c.id=u.cat_id where u.user_pub_id="' + user_pub_id + '"',//,GROUP_CONCAT(cat.id) AS cat_id
    var que="SELECT cs.currency_symbol,multcat.user_pub_id,multcat.cat_id,cat.cat_name,us.*,concat('"+base_url+"',us.image) as image,GROUP_CONCAT(cat.cat_name) AS cat_name,GROUP_CONCAT(cat.id) AS cat_id from user us JOIN `currency_setting` AS cs ON cs.id = us.currency_id JOIN `multiple_category` AS multcat ON multcat.user_pub_id = us.user_pub_id JOIN `categories` AS cat ON cat.id = multcat.cat_id WHERE us.status = '1' AND us.is_artist = '1' AND us.user_pub_id !='"+user_pub_id+"' AND (us.sub_end_date != '' AND us.sub_end_date > ROUND(UNIX_TIMESTAMP()*1000)) GROUP BY us.user_pub_id";
...
{
    var que = "SELECT cs.currency_symbol,multcat.user_pub_id,multcat.cat_id,cat.cat_name,us.*,concat('"+base_url+"',us.image) as image,concat('',cat.id) as cat_id,"+
    ...
    "JOIN `multiple_category` AS multcat ON multcat.user_pub_id = us.user_pub_id JOIN `categories` AS cat ON cat.id = multcat.cat_id WHERE us.status = '1' HAVING distance <= '32'AND us.is_artist = '1' AND us.user_pub_id !='"+user_pub_id+"' "+
...
module.exports.getPostedJobs=function(user_pub_id,cb){
    var que='SELECT us.name,us.mobile_no,cs.currency_symbol,pj.*,cat.cat_name,cat.image as cat_image,concat("'+base_url+'",us.image) as image,GROUP_CONCAT(cat.cat_name ORDER BY cat.id) as cat_name,GROUP_CONCAT(cat.image ORDER BY cat.id) as cat_image,concat("'+base_url+'",cat.image) as cat_image from post_job as pj join categories as cat on FIND_IN_SET(cat.id,pj.cat_id) join user as us on us.user_pub_id=pj.user_pub_id join currency_setting as cs on cs.status=1 WHERE pj.status !="4" AND pj.user_pub_id="'+user_pub_id+'" GROUP BY pj.job_id ORDER BY pj.id DESC';
...
module.exports.getPostedJobs1=function(user_pub_id,cb){
    var que='SELECT us.name,us.mobile_no,cs.currency_symbol,cs.currency_name,pj.*,cat.cat_name,cat.image as cat_image,concat("'+base_url+'",us.image) as image,GROUP_CONCAT(cat.cat_name ORDER BY cat.id) as cat_name,GROUP_CONCAT(cat.image ORDER BY cat.id) as cat_image,concat("'+base_url+'",cat.image) as cat_image from post_job as pj join categories as cat on FIND_IN_SET(cat.id,pj.cat_id) join user as us on us.user_pub_id=pj.user_pub_id join currency_setting as cs on cs.id=pj.currency_id WHERE pj.status !="4" AND pj.user_pub_id="'+user_pub_id+'" GROUP BY pj.job_id ORDER BY pj.id DESC limit 5';
    // var que='SELECT sk.skill,cat.cat_name,us.name,us.mobile_no,cs.currency_symbol,pj.* from post_job as pj join skill as sk on sk.id=pj.skill_id JOIN categories as cat on cat.id=pj.cat_id JOIN user as us on us.user_pub_id=pj.user_pub_id JOIN currency_setting as cs on cs.status=1 WHERE pj.status!="4" AND pj.user_pub_id="'+user_pub_id+'" ORDER BY pj.id DESC';
    con.query(que,cb);
}
module.exports.getAcceptedJob=function(user_pub_id,cb)
{
    // var que='SELECT us.name,us.mobile_no,cs.currency_symbol,pj.*,cat.cat_name,concat("'+base_url+'",us.image) as image,GROUP_CONCAT(cat.cat_name ORDER BY cat.id) as cat_name from post_job as pj join categories as cat on FIND_IN_SET(cat.id,pj.cat_id) join user as us on us.user_pub_id=pj.user_pub_id join currency_setting as cs on cs.status=1 WHERE pj.status !="4" AND pj.user_pub_id="'+user_pub_id+'" GROUP BY pj.job_id ORDER BY pj.id DESC';
    ...
    var que="select p.*,a.*,c.cat_name, u.name,cs.currency_symbol,cs.currency_name,concat('"+base_url+"',u.image)  as image,GROUP_CONCAT(c.cat_name ORDER BY c.id) as cat_name from post_job p join categories c on FIND_IN_SET(c.id,p.cat_id) join user u on  u.user_pub_id = p.user_pub_id  join currency_setting cs on cs.id=p.currency_id join apply_job a on p.job_id=a.job_id where a.artist_pub_id='"+user_pub_id+"' and a.status='1' GROUP BY p.job_id order by a.created_at desc limit 5";
    con.query(que,cb);
}
...
{
    var que = "SELECT * FROM `notification` WHERE `user_pub_id` = '"+user_pub_id+"' OR `type` = '10000' ORDER BY `created_at` DESC";    //console.log(que);
    con.query(que, cb);
}
...
module.exports.GetInvoiceByJobId=function(job_id,cb)
{
    // var que='select * from booking_invoice1 where job_id="'+job_id+'"'
    var que="select u.name,concat('"+base_url+"',u.image)as image,usr.name user_name,concat('"+base_url+"',usr.image)as user_image,cs.currency_symbol,p.start_time,p.cat_id,cat.cat_name,p.end_time,p.duration,GROUP_CONCAT(cat.cat_name) as cat_name,co.*,b.*, IF(co.discount_type!='',IF(co.discount_type=2, concat(co.discount,'Flat Off'), concat(co.discount,'% Off')),'') as discount from booking_invoice1 as b join user as u on u.user_pub_id=b.artist_pub_id join user as usr on usr.user_pub_id=b.user_pub_id left join coupon as co on co.id=b.coupon_id JOIN `currency_setting` AS cs ON cs.id =b.currency_id join post_job as p on p.job_id=b.job_id JOIN `categories` AS cat ON FIND_IN_SET(cat.id,p.cat_id) where b.job_id='"+job_id+"'"
    con.query(que,cb)
}
module.exports.GetInvoice=function(user_pub_id,status,cb)
{
    //var que="select p.*,a.*,c.cat_name, u.name,cs.currency_symbol,concat('"+base_url+"',u.image)  as image,GROUP_CONCAT(c.cat_name ORDER BY c.id) as cat_name from post_job p join categories c on FIND_IN_SET(c.id,p.cat_id) join user u on  u.user_pub_id = p.user_pub_id  join currency_setting cs on cs.status='1' join apply_job a on p.job_id=a.job_id where a.artist_pub_id='"+user_pub_id+"' and a.status='1' GROUP BY p.job_id order by a.created_at desc limit 5"; 
    ...
    var que="select u.name,concat('"+base_url+"',u.image)as image,usr.name user_name,concat('"+base_url+"',usr.image)as user_image,cs.currency_symbol,cs.currency_name,p.cat_id,cat.cat_name,p.start_time,p.end_time,p.duration,GROUP_CONCAT(cat.cat_name) as cat_name,co.*, b.*, IF(co.discount_type!='',IF(co.discount_type=2, concat(co.discount,'Flat Off'), concat(co.discount,'% Off')),'') as discount from booking_invoice1 as b join user as u on u.user_pub_id=b.artist_pub_id join user as usr on usr.user_pub_id=b.user_pub_id left join coupon as co on co.id=b.coupon_id JOIN `currency_setting` AS cs ON cs.id = b.currency_id join post_job as p on p.job_id=b.job_id JOIN `categories` AS cat ON FIND_IN_SET(cat.id,p.cat_id) where (b.user_pub_id='"+user_pub_id+"' || b.artist_pub_id='"+user_pub_id+"') and b.status='"+status+"' GROUP BY p.job_id order by b.created_at DESC"
    con.query(que,cb)
}
...
module.exports.GetInvoice1=function(user_pub_id,cb)
{
    ...
    var que="select u.name,concat('"+base_url+"',u.image)as image,cs.currency_symbol,cs.currency_name,p.start_time,p.end_time,p.duration,co.*,b.*, IF(co.discount_type!='',IF(co.discount_type=2, concat(co.discount,'Flat Off'), concat(co.discount,'% Off')),'') as discount from booking_invoice1 as b join user as u on u.user_pub_id=b.artist_pub_id left join coupon as co on co.id=b.coupon_id JOIN `currency_setting` AS cs ON cs.id = b.currency_id join post_job as p on p.job_id=b.job_id where b.user_pub_id='"+user_pub_id+"' or b.artist_pub_id ='"+user_pub_id+"'  order by b.created_at DESC"
    con.query(que,cb)
}
module.exports.GetInvoice12=function(user_pub_id,cb)
{
//1. var que="select u.name,concat('"+base_url+"',u.image)as image,cs.currency_symbol,c.cat_name,p.start_time,p.end_time,p.duration,co.*,b.*, IF(co.discount_type!='',IF(co.discount_type=2, concat(co.discount,'Flat Off'), concat(co.discount,'% Off')),'') as discount from booking_invoice1 as b join user as u on u.user_pub_id=b.artist_pub_id left join coupon as co on co.id=b.coupon_id JOIN `currency_setting` AS cs ON cs.status =1 join categories as c on c.id=u.cat_id join post_job as p on p.job_id=b.job_id where b.user_pub_id='"+user_pub_id+"' or b.artist_pub_id ='"+user_pub_id+"'  order by b.created_at DESC LIMIT 5"
//2. var que="select u.name,concat('"+base_url+"',u.image)as image,cs.currency_symbol,p.start_time,p.cat_id,cat.cat_name,p.end_time,p.duration,GROUP_CONCAT(cat.cat_name) as cat_name,co.*,b.*, IF(co.discount_type!='',IF(co.discount_type=2, concat(co.discount,'Flat Off'), concat(co.discount,'% Off')),'') as discount from booking_invoice1 as b join user as u on u.user_pub_id=b.artist_pub_id left join coupon as co on co.id=b.coupon_id JOIN `currency_setting` AS cs ON cs.status =1 join post_job as p on p.job_id=b.job_id JOIN `categories` AS cat ON FIND_IN_SET(cat.id,p.cat_id) where b.user_pub_id='"+user_pub_id+"' or b.artist_pub_id ='"+user_pub_id+"' GROUP BY p.job_id order by b.created_at DESC LIMIT 5"
    var que="select u.name,concat('"+base_url+"',u.image)as image,usr.name user_name,concat('"+base_url+"',usr.image)as user_image,cs.currency_symbol,p.start_time,p.cat_id,cat.cat_name,p.end_time,p.duration,GROUP_CONCAT(cat.cat_name) as cat_name,co.*,b.*, IF(co.discount_type!='',IF(co.discount_type=2, concat(co.discount,'Flat Off'), concat(co.discount,'% Off')),'') as discount from booking_invoice1 as b join user as u on u.user_pub_id=b.artist_pub_id join user as usr on usr.user_pub_id=b.user_pub_id left join coupon as co on co.id=b.coupon_id JOIN `currency_setting` AS cs ON cs.status =1 join post_job as p on p.job_id=b.job_id JOIN `categories` AS cat ON FIND_IN_SET(cat.id,p.cat_id) where b.user_pub_id='"+user_pub_id+"' or b.artist_pub_id ='"+user_pub_id+"' GROUP BY p.job_id order by b.created_at DESC LIMIT 5"
    con.query(que,cb)
}
						...
						return new Promise(function(resolve, reject) {
							...
							con.query('select coalesce(count(job_id),0) as applicant_count from apply_job where job_id="'+resultdata.job_id+'" and status in(0,1,2,5)',function(err,resultCount){
                                if(err){
                                    ...
                                    con.query('select * from currency_setting where id="' + resultdata.currency_id + '"', function (err, resultCurrency) {
                                        if(resultCurrency == null || resultCurrency == '' || resultCurrency == undefined || resultCurrency == []){
                                            resultdata.currency_name = '';
                                            resultdata.currency_symbol = '';
                                        }else{
                                            resultdata.currency_name = resultCurrency[0].currency_name;
                                            resultdata.currency_symbol = resultCurrency[0].currency_symbol;
                                        }
                                        var is_invoice = 0;
                                        var is_rating = 0;
                                        con.query("select u.name,concat('"+base_url+"',u.image)as image,usr.name user_name,concat('"+base_url+"',usr.image)as user_image,cs.currency_symbol,p.start_time,p.cat_id,cat.cat_name,p.end_time,p.duration,GROUP_CONCAT(cat.cat_name) as cat_name,co.*,b.*, IF(co.discount_type!='',IF(co.discount_type=2, concat(co.discount,'Flat Off'), concat(co.discount,'% Off')),'') as discount from booking_invoice1 as b join user as u on u.user_pub_id=b.artist_pub_id join user as usr on usr.user_pub_id=b.user_pub_id left join coupon as co on co.id=b.coupon_id JOIN `currency_setting` AS cs ON cs.status =1 join post_job as p on p.job_id=b.job_id JOIN `categories` AS cat ON FIND_IN_SET(cat.id,p.cat_id) where b.job_id='"+resultdata.job_id+"'",function(err,invoice_data){
                                            if(invoice_data == null || invoice_data == '' || invoice_data == undefined || invoice_data == [])invoice_data = {};
                                            else {
                                                if(invoice_data[0].job_id == null)invoice_data = {}
                                                else{
                                                    invoice_data = invoice_data[0];
                                                    is_invoice = 1;
                                                }
                                            }
                                            resultdata.invoce_data = invoice_data;
                                            if(is_invoice == 1){
                                                var que="select r.*,u.name,concat('"+base_url+"',u.image)as image from rating as r join user as u on u.user_pub_id=r.user_pub_id where r.invoice_id='"+invoice_data.invoice_id+"'";
                                                con.query(que,function(err,rating_data){
                                                    if(rating_data == null || rating_data == '' || rating_data == undefined || rating_data == []){
                                                        rating_data = [];
                                                    }else{
                                                        var is_rating_data =  rating_data.filter(function(item) {
                                                            return item.user_pub_id == result[0].user_pub_id;
                                                        });
                                                        if(is_rating_data == null || is_rating_data == '' || is_rating_data == undefined)is_rating = 0
                                                        else is_rating = 1
                                                    }
                                                    resultdata.is_rating = is_rating;
                                                    resultdata.rating_data = rating_data;
                                                    result_array.push(resultdata);
                                                    resolve(result_array);
                                                })
                                            }else{
                                                resultdata.is_rating = 0;
                                                resultdata.rating_data = [];
                                                result_array.push(resultdata);
                                                resolve(result_array);
                                            }
                                        })
...
module.exports.getJobsArtist=function(user_pub_id,cat_id,job_id,Latitude,Longitude,cb)
{
    ...
    var distance = '111.111 * DEGREES(ACOS(LEAST(1.0, COS(RADIANS('+Latitude+')) * COS(RADIANS(Pj.Latitude)) * COS(RADIANS('+Longitude+' - Pj.Longitude)) + SIN(RADIANS('+Latitude+')) * SIN(RADIANS(Pj.Latitude))))) AS distance_in_km'
    if(Latitude==null || Latitude == '' || Latitude == undefined || Longitude==null || Longitude == '' || Longitude == undefined){
        var que ="SELECT cat.cat_name,us.name AS name,us.mobile_no AS mobile_no,us.address AS address,f.status as favourite_status ,COALESCE(f.status,0) as favourite_status,(Pj.duration * aus.price) AS price,CONCAT( '"+constant.base_url+"',  us.image) AS image ,cs.currency_symbol,Pj.* FROM `post_job` AS Pj JOIN `categories` AS cat ON FIND_IN_SET(cat.id,Pj.cat_id) JOIN `user` AS us ON us.user_pub_id = Pj.user_pub_id JOIN `user` AS aus ON aus.user_pub_id = '"+user_pub_id+"' JOIN `currency_setting` AS cs ON cs.status = 1 left join favourite f on f.target_id=Pj.job_id and f.user_pub_id='"+user_pub_id+"' where Pj.status IN (0) and Pj.cat_id ='"+cat_id+"' and Pj.job_id!='"+job_id+"' and Pj.user_pub_id !='"+user_pub_id+"'  and Pj.job_id not in ( select job_id from apply_job where artist_pub_id ='"+user_pub_id+"') GROUP BY Pj.job_id order by created_at DESC";
    }else{
        var que ="SELECT cat.cat_name,us.name AS name,us.mobile_no AS mobile_no,"+distance+",us.address AS  address,f.status as favourite_status ,COALESCE(f.status,0) as favourite_status,(Pj.duration * aus.price) AS price,CONCAT( '"+constant.base_url+"',  us.image) AS image ,cs.currency_symbol,Pj.* FROM `post_job` AS Pj JOIN `categories` AS cat ON FIND_IN_SET(cat.id,Pj.cat_id) JOIN `user` AS us ON us.user_pub_id = Pj.user_pub_id JOIN `user` AS aus ON aus.user_pub_id = '"+user_pub_id+"' JOIN `currency_setting` AS cs ON cs.status = 1 left join favourite f on f.target_id=Pj.job_id and f.user_pub_id='"+user_pub_id+"' where Pj.status IN (0) and Pj.cat_id ='"+cat_id+"' and Pj.job_id!='"+job_id+"' and Pj.user_pub_id !='"+user_pub_id+"'  and Pj.job_id not in ( select job_id from apply_job where artist_pub_id ='"+user_pub_id+"') GROUP BY Pj.job_id order by created_at DESC";
    }
    con.query(que, cb);
}   
module.exports.getJobsArtist1=function(user_pub_id,Latitude,Longitude,cb)
{
    ...
    var distance = '111.111 * DEGREES(ACOS(LEAST(1.0, COS(RADIANS('+Latitude+')) * COS(RADIANS(Pj.Latitude)) * COS(RADIANS('+Longitude+' - Pj.Longitude)) + SIN(RADIANS('+Latitude+')) * SIN(RADIANS(Pj.Latitude))))) AS distance_in_km'
    if(Latitude==null || Latitude == '' || Latitude == undefined || Longitude==null || Longitude == '' || Longitude == undefined){
        var que="SELECT cat.cat_name,us.name AS name,us.mobile_no AS mobile_no,COALESCE(f.status,0) as favourite_status,us.address AS address,(Pj.duration * us.price) AS price,CONCAT( '"+constant.base_url+"',  us.image) AS image ,cs.currency_symbol,Pj.* FROM `post_job` AS Pj  JOIN `categories` AS cat ON FIND_IN_SET(cat.id,Pj.cat_id)  JOIN `user` AS us ON us.user_pub_id = Pj.user_pub_id JOIN `currency_setting` AS cs ON cs.status = 1 left join favourite f on f.target_id=Pj.job_id  and f.user_pub_id='"+user_pub_id+"' where Pj.status IN (0) and  Pj.user_pub_id !='"+user_pub_id+"'  and  Pj.job_id not in ( select job_id from apply_job where artist_pub_id ='"+user_pub_id+"') GROUP BY Pj.job_id order by created_at DESC";
    }else {
        var que="SELECT cat.cat_name,us.name AS name,us.mobile_no AS mobile_no,"+distance+",COALESCE(f.status,0) as favourite_status,us.address AS address,(Pj.duration * us.price) AS price,CONCAT( '"+constant.base_url+"',  us.image) AS image ,cs.currency_symbol,Pj.* FROM `post_job` AS Pj  JOIN `categories` AS cat ON FIND_IN_SET(cat.id,Pj.cat_id)  JOIN `user` AS us ON us.user_pub_id = Pj.user_pub_id JOIN `currency_setting` AS cs ON cs.status = 1 left join favourite f on f.target_id=Pj.job_id  and f.user_pub_id='"+user_pub_id+"' where Pj.status IN (0) and  Pj.user_pub_id !='"+user_pub_id+"'  and  Pj.job_id not in ( select job_id from apply_job where artist_pub_id ='"+user_pub_id+"') GROUP BY Pj.job_id order by created_at DESC";
    }
    con.query(que,cb);
}
...
module.exports.getJobsAppliedUser=function(job_id,cb)
{
    ...
    var que ="SELECT cat.cat_name,Aj.status As status,Aj.id As aj_id,us.name AS name,us.mobile_no AS mobile_no,us.address AS address,(Pj.duration * us.price) AS price,CONCAT( '"+base_url+"',  us.image) AS image,GROUP_CONCAT(cat.cat_name ORDER BY cat.id) as cat_name,cs.currency_symbol,cs.currency_name,Pj.*,Aj.artist_pub_id FROM `apply_job` AS Aj JOIN `post_job` AS Pj ON Pj.job_id = Aj.job_id  JOIN `categories` AS cat ON FIND_IN_SET(cat.id,Pj.cat_id) JOIN `user` AS us ON us.user_pub_id = Aj.artist_pub_id JOIN `currency_setting` AS cs ON cs.id = Pj.currency_id where Aj.job_id='"+job_id+"' and Aj.status != '3' GROUP BY Pj.job_id";
    con.query(que, cb);
} 
...
module.exports.getCurrentAppointmentArtist=function(user_pub_id,cb)
{
    ...
    var que ="SELECT cat.cat_name,us.name AS name,us.mobile_no AS mobile_no,CONCAT( '"+constant.base_url+"',  us.image) AS image,GROUP_CONCAT(cat.cat_name ORDER BY cat.id) as cat_name,cs.currency_symbol,cs.currency_name,p.*,a.artist_pub_id FROM `post_job` AS p JOIN `categories` AS cat ON FIND_IN_SET(cat.id,p.cat_id) JOIN user AS us ON us.user_pub_id = p.user_pub_id JOIN `currency_setting` AS cs ON cs.id = p.currency_id join apply_job a on a.job_id=p.job_id WHERE p.status IN (5) AND a.artist_pub_id ='"+user_pub_id+"' GROUP BY p.job_id";
    con.query(que, cb);
}
...
{
    var api_key = constant.mailgun_api_key//'ef5924af46ea9c61c8e31f5c006ada8b-65b08458-d9013671';
    var domain = constant.mailgun_domain //'mg.live2talks.com';
    con.query("select * from api_keys where api_name='Mailgun'", function (err, Mailgun) {
...
module.exports.saveAndSendNotification = async function(to_user_id,title,message,type,device_token){
    var data = {
        // id:uniqid(),
    ...
    con.query('insert into notification set ?', [data], function (err, result) {
        if (err) {
          console.log(err);
        } else {
...
module.exports.saveAndSendNotificationOfTicket=async function(to_user_id,title,message,type){
    var data = {
        // id:uniqid(),
        user_pub_id: to_user_id,
        title: title,
...
{
 var que="select r.*,p.price,cs.currency_symbol,u.name,concat('"+base_url+"',u.image)as image from rating as r join booking_invoice1 as b on b.invoice_id=r.invoice_id join user as u on u.user_pub_id=r.artist_pub_id JOIN `currency_setting` AS cs ON cs.id =b.currency_id join post_job as p on p.job_id=b.job_id where r.user_pub_id='"+user_pub_id+"' and p.status=2 order by created_at"
    con.query(que,cb);
}

module.exports.getRatingByInvoiceId=function(invoice_id,cb)
{
 var que="select r.*,u.name,concat('"+base_url+"',u.image)as image from rating as r join user as u on u.user_pub_id=r.user_pub_id where r.invoice_id='"+invoice_id+"'";
    con.query(que,cb);
}
...
//    +"WHERE c.user_pub_id='"+user_pub_id+"' GROUP BY c.artist_pub_id ORDER BY send_at DESC  ";
    +"WHERE c.user_pub_id='"+user_pub_id+"' || c.artist_pub_id='"+user_pub_id+"' GROUP BY c.thread_id ORDER BY send_at DESC  ";
...
module.exports.deleteCategoryInUser=function(user_pub_id, cb)
{
    var que = "DELETE FROM multiple_category WHERE user_pub_id='"+user_pub_id+"'";
...
module.exports.updateCategoryInUser=function(user_pub_id, array, cb)
{
    var date = new Date();
    var qry = 'INSERT INTO multiple_category (user_pub_id,cat_id) values ?';
    con.query(qry,[array],function(err,result){
...
module.exports.getMyChatData=function(user_pub_id,artist_pub_id,cb){
    var que = 'select * from chat where (user_pub_id="'+user_pub_id+'" AND artist_pub_id="'+artist_pub_id+'") OR (user_pub_id="'+artist_pub_id+'" AND artist_pub_id="'+user_pub_id+'")';
    con.query(que, cb);
} 