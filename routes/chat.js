...
var Chat = multer.diskStorage({ //multers disk storage settings
    destination: function(req, file, cb) {
        cb(null, '../../../../../var/www/html/Bakvebul/chat_media');
    },
    filename: function(req, file, cb) {
        var datetimestamp = Date.now();
        cb(null, file.fieldname + '-' + datetimestamp + '.' + file.originalname.split('.')[file.originalname.split('.').length - 1]);
...
app.post("/upload_image",upload_chat.single('image'),function(req,res){
    if(req.file){
        var data = base_url+"Bakvebul/chat_media/"+req.file.filename;
        cm.responseMessagedata(constant.One,constant.success,data,res)
    }
})
    ...
    if(!req.body.user_pub_id||!req.body.artist_pub_id||!req.body.chat_type){
        ...
        if(req.body.chat_type==1){
            ...
            var parData={
                    ...
                    message:req.body.message,
                    send_at:(new Date()).valueOf().toString(),
                    image:base_url+"Bakvebul/chat_media/"+req.files[0].filename,
                }
